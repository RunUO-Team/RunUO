						...
						foreach(DataItem item in DataPoints)
						{
							using(SolidBrush barBrush = new SolidBrush(item.ItemColor))
							{
								...
								else if ( _renderMode == BarGraphRenderMode.Bars )
								{
									...
									using ( SolidBrush ltBrsh = new SolidBrush( Color.FromArgb((int)(item.ItemColor.R * 1.1f), (int)(item.ItemColor.G * 1.1f), (int)(item.ItemColor.B * 1.1f)) ) )
										graph.FillPolygon( ltBrsh, new PointF[]{ pts[0], pts[2], pts[5], pts[4] } );

									using ( SolidBrush drkBrush = new SolidBrush( Color.FromArgb((int)(item.ItemColor.R * 0.95f), (int)(item.ItemColor.G * 0.95f), (int)(item.ItemColor.B * 0.95f)) ) )
