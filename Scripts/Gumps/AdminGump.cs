	...
	public class AdminGump : Gump
	{
		...
		public static void Marked_Callback( Mobile from, bool okay, object state )
		{
			...
			if ( okay )
			{
				...
				from.SendGump( new NoticeGump( 1060637, 30720, $"You have {(ban ? "banned" : "deleted")} the account{(rads.Count == 1 ? "" : "s")}.", 0xFFC000, 420, 280, new NoticeGumpCallback( ResendGump_Callback ), new object[]{ list, rads, ban ? page : 0 } ) );
			...
			{
				from.SendGump( new NoticeGump( 1060637, 30720, $"You have chosen not to {(ban ? "ban" : "delete")} the account{(rads.Count == 1 ? "" : "s")}.", 0xFFC000, 420, 280, new NoticeGumpCallback( ResendGump_Callback ), new object[]{ list, rads, page } ) );
			}
		}
			...
			switch ( type )
			{
				...
				case 5:
				{
					switch ( index )
					{
						...
						case 18: // firewall all
						{
							...
							{
								from.SendGump( new WarningGump( 1060635, 30720, $"You are about to firewall {a.LoginIPs.Length} address{(a.LoginIPs.Length != 1 ? "s" : "")}. Do you wish to continue?", 0xFFC000, 420, 400, new WarningGumpCallback( FirewallShared_Callback ), a ) );
							}
							else
							{
								from.SendGump( new AdminGump( from, AdminGumpPage.AccountDetails_Access_ClientIPs, 0, null, "This account has not yet been accessed.", m_State ) );
						...
						case 27: // Ban marked
						{
							...
							if ( rads.Count > 0 )
								from.SendGump( new WarningGump( 1060635, 30720, $"You are about to ban {rads.Count} marked account{(rads.Count == 1 ? "" : "s")}. Be cautioned, the only way to reverse this is by hand--manually unbanning each account.<br><br>Do you wish to continue?", 0xFFC000, 420, 280, new WarningGumpCallback( Marked_Callback ), new object[]{ true, list, rads, m_ListPage } ) );
							else
						...
						case 28: // Delete marked
						{
							...
							if ( rads.Count > 0 )
								from.SendGump( new WarningGump( 1060635, 30720, $"You are about to <em><basefont color=red>permanently delete</basefont></em> {rads.Count} marked account{(rads.Count == 1 ? "" : "s")}. Likewise, all characters on the account{(rads.Count == 1 ? "" : "s")} will be deleted, including equiped, inventory, and banked items. Any houses tied to the account{(rads.Count == 1 ? "" : "s")} will be demolished.<br><br>Do you wish to continue?", 0xFFC000, 420, 280, new WarningGumpCallback( Marked_Callback ), new object[]{ false, list, rads, m_ListPage } ) );
							else
								from.SendGump( new NoticeGump( 1060637, 30720, "You have not yet marked any accounts. Place a check mark next to the accounts you wish to ban and then try again.", 0xFFC000, 420, 280, new NoticeGumpCallback( ResendGump_Callback ), new object[]{ list, rads, m_ListPage } ) );
						...
						case 36: // Clear login addresses
						{
							...
							if ( ips.Length == 0 )
								...
								from.SendGump( new WarningGump( 1060635, 30720, $"You are about to clear the address list for account {a} containing {ips.Length} {(ips.Length == 1 ? "entry" : "entries")}. Do you wish to continue?", 0xFFC000, 420, 280, new WarningGumpCallback( RemoveLoginIPs_Callback ), a ) );
				...
				case 8:
				{
					if ( m_List != null && index >= 0 && index < m_List.Count )
					{
						...
						{
							from.SendGump( new WarningGump( 1060635, 30720, $"You are about to firewall {m_List[index]}. All connection attempts from a matching IP will be refused. Are you sure?", 0xFFC000, 420, 280, new WarningGumpCallback( Firewall_Callback ), new object[]{ a, m_List[index] } ) );
						}
				...
				case 10:
				{
					if ( m_List != null && index >= 0 && index < m_List.Count )
					{
						if ( m_PageType == AdminGumpPage.AccountDetails_Access_ClientIPs )
						{
							...
							from.SendGump( new WarningGump( 1060635, 30720, $"You are about to remove address {ip} from account {a}. Do you wish to continue?", 0xFFC000, 420, 280, new WarningGumpCallback( RemoveLoginIP_Callback ), new object[] { a, ip } ) );
						}
					}
